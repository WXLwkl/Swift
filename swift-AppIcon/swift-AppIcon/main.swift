        ...
        do {
            ...
            if typeAndTypes.type != .errorType {
                ...
                appIcon.images = appIcon.images.filter {
                    if (typeAndTypes.type.contains(.iPhone) || typeAndTypes.types.contains(.iPad)), $0.idiom == "ios-marketing" {
                        return true
                    }
                    if typeAndTypes.types.contains(.appleWatch), $0.idiom == "watch-marketing" {
                        return true
                    }
...
func createFile(appIcon: AppIcon, image: CGImage) {
    ...
    do {
        ...
        print("文件目录\(filePath)创建失败: \(error.localizedDescription)")
    }
}
...
func start() {
    print("请输入图片名:")
    if let inputPath = readLine() {
        if inputPath.count == 0 {
            return print("请检查图片路径地址是否正确")
        }
