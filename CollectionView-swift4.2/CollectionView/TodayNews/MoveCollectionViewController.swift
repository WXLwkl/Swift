...
private let kHeaderReuseIdentifier = "kHeaderReuseIdentifier"
...
class MoveCollectionViewController: UIViewController {
    ...
    lazy var collectionView: UICollectionView = { [weak self] in
        ...
        // 注册cell和头部视图
        collectionView.register(UINib.init(nibName: "MoveCollectionViewCell", bundle: nil), forCellWithReuseIdentifier: kCellReuseIdentifier)
        collectionView.register(UINib.init(nibName: "MoveHeaderCollectionReusableView", bundle: nil), forSupplementaryViewOfKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: kHeaderReuseIdentifier)
    ...
    func handleGesture(_ sender: UIGestureRecognizer) {
        ...
        case .began:
            ...
            guard let letOriginalIndexPath = originalIndexPath, originalIndexPath?.section == 0, snapshot == nil else { return }
            // 如果还没进入编辑状态，就让他进入编辑状态，cell同时显示移除按钮
            if !isEdit {
                isEdit = true
                navigationItem.rightBarButtonItem?.title = "完成"
                // 这里不用刷新，是因为如果刷新，就不能根据indexpath 找到cell
        ...
        case .ended:
            guard let letOriginalIndexPath = originalIndexPath, snapshot != nil else { return }
...
extension MoveCollectionViewController: UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: kCellReuseIdentifier, for: indexPath) as! MoveCollectionViewCell
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        let headReuseView = collectionView.dequeueReusableSupplementaryView(ofKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: kHeaderReuseIdentifier, for: indexPath) as! MoveHeaderCollectionReusableView
        if indexPath.section == 0 {
